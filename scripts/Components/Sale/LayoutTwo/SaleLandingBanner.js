...
const ButtonLink = ({ settings }) => {
  ...
  return (
    <div className="p-5 justify-items-center md:w-98 lg:w-110 lg:px-12">
      <div className={`px-4 lg:px-10 bg-${bg_color} ${outline && 'outline-blue'}`}>
        <div
        ...
        <div
          className={`center mb-2 mt-2 font-normal font-serif text-8xl lg:text-9xl ${discount_percent_color}`}
        >
          <span>{discount}%</span>
          <span className="inline-block uppercase font-base font-medium text-5.25xl lg:text-7xl lg:font-semibold transform -translate-x-1.5">
            ff
          </span>
        </div>

        <div className={`mb-2 md:mb-8 center ${discount_text_color} ${discount_text_font_size}`}>
          <span
            className={`${discount_text_font_size_mobile} lg:${discount_text_font_size} md:${discount_text_font_size_tablet} font-normal tracking-wider`}
          >
            {discount_text}{' '}
          </span>
