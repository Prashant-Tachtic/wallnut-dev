...
const ButtonLink = ({ settings }) => {
  const {
    ...
    collection_hero_title_font,
    collection_hero_secondary_title_font,
  } = settings;

  return (
    <div className="justify-items-center lg:px-36 md:col-span-12 md:p-10 px-5 py-8">
      <div className="bg-offwhite-cream bg-opacity-70 lg:px-10 max-w-screen-xxl mx-auto px-4">
        <div className="md:flex md:justify-center md:pb-2 md:pt-12">
          <div className="flex justify-center center font-medium font-serif lg:font-normal lg:text-8xl mb-2 md:mb-0 md:pb-0 md:pt-0 pt-12 text-6xl text-orange-burnt">
            <span className="text-5xl justify-center mt-3 mr-2">$</span>
            <span className="font-bold tracking-wide">{discount}</span>
            <span className="font-bold text-blue uppercase">OFF</span>
          </div>

          <div className="center font-sans font-normal lg:pl-6 lg:pt-7 lg:text-5xl md:pl-3 md:pt-5 pb-4 text-1xl text-3xl">
            <span className={`${collection_hero_title_font} text-blue`}>
              {collection_hero_title}
            </span>
          </div>
        </div>

        <div className="center whitespace-no-wrap leading-5 lg:text-2.5xl mb-4 text-base text-blue">
          <span className={`${collection_hero_secondary_title_font}`}>
            {collection_hero_secondary_title}{' '}
          </span>
          <span className="font-medium text-orange-burnt">{discount_code}</span>
        </div>
...
const SaleCollectionsBanner = ({ settings }) => {
  const {
    ...
    collection_hero_mobile_image,
    collection_hero_tablet_image,
    collection_hero_toggle,
  } = settings;

  if (!collection_hero_toggle) {
    return null;
  }
...
SaleCollectionsBanner.propTypes = {
  settings: PropTypes.shape({
    collection_hero_toggle: PropTypes.string,
