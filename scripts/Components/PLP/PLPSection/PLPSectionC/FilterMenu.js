...
const FilterMenu = ({
  ...
  if (type === 'colorpicker') {
    return (
      <MenuContainer hasOneRow={hasOneRow} id="filterMenu" {...{ menuOpen, menuName }}>
...
FilterMenu.propTypes = {
  ...
  hasOneRow: PropTypes.bool,
};

const MenuContainer = styled.div(({ menuOpen, menuName, hasOneRow }) => {
  if (hasOneRow) {
    return [
      tw`w-full left-0 sm:left-0 md:left-auto z-50 bg-white md:w-68 absolute`,
      !menuOpen && tw`hidden`,
      menuOpen && tw`visible`,
      menuName === 'collection' && tw` transform translate-y-4 md:translate-y-4.5 py-4 px-4`,
      menuName === 'type' &&
        tw`transform -translate-x-33 translate-y-4 md:translate-y-4.5 py-4 px-4`,
      menuName === 'colorpicker' && tw`transform translate-y-4 md:translate-y-4.5 px-3 py-3.5`,
  ...
  return [
    ...
    menuName === 'type' &&
      tw`transform -translate-x-33 translate-y-15 md:translate-y-4.5 py-4 px-4`,
