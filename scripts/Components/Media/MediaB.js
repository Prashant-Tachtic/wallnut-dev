...
const Media = ({
  ...
  if (backgroundImage && overlayImage && overlayImageMobile) {
    return (
...
const ImageContainerWithOverlay = styled.span.attrs(({ bgCover }) => {
  let className = 'bg-center bg-no-repeat h-full inline-block';
  ...
  @media (max-width: 1240px) {
    background-image: url(${({ overlayImageMobile }) => overlayImageMobile}),
      url(${({ image }) => image});
  }
  @media (min-width: 1240px) {
    background-image: url(${({ overlayImage }) => overlayImage}), url(${({ image }) => image});
  }
  width: ${({ width }) => width};
`;
